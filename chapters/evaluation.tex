...
Our performed tasks involved the execution of five different exercises which consisted of rather simple combinations of possible anatomic arm movements, described in section \ref{vision-feedback}.
...
The test users profile gathered from the questionnaire is available in \todo{colocar o questionario em apendice}.
...
However, we observed users gained more interest in using SleeveAr for being something interactive and a completely different experience from what they used to.
...
In Table \ref{table:widgets}, the resulting values from our participants are depicted. \todo{o que concluir destes valores?}
Participants were also free to share any personal thoughts regarding every visual feedback presented during the tests.
...
On the other hand, the arm projection, even though being considered a very useful idea, received some suggestions regarding our implementation. Some participants stated some difficulty following both the arm and floor feedback at the same time, even though they are placed in the some field of view.
...
This could be solved by positioning the floor feedback further away from the user, and will be discussed at Section \ref{sec:futurework}.
...
It is measured using the \textbf{\ac{DTW}} \todo{referencia para o algoritmo} algorithm, which is appropriate for measuring a degree of similarity between two temporal sequences which may vary in time or speed. 
