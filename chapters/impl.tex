...
Audio feedback was used for simple notifications. Events that require the user attention, we \todo{Falta completar}
...
With this selection, we were able to receive tracking data from the three locations, and therefore, replicate the arm with data \todo{what is it mean with "replicate the arm with data"? reformulate}. In figure \ref{fig:rigidbodiesattached} we can observe
...
Looking at \todo{fig 1}, a cube inside the room is being tracked by the Optitrack sensor. Hence, we receive its raw positional data, i.e., its \ac{VP}, which is calculated based on the tracking system coordinates' origin.
...
Even though we might have the circle being projected perfectly below \todo{fig 4, A}, it was not constantly synced.
If we moved the cube to another \ac{PP}, closer to the edges of the projected area, like in cubes B and C from \todo{fig 4}, the circle would not remain below it. 
...
\begin{figure}[!t]
    ...
    \end{center}
    \caption{Recording Interface \todo{check caption}}
    \label{fig:recordinginterface}
\end{figure}

\todo{Here user should be replaced by therapist over the text?}
